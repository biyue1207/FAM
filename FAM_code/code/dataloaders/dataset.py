...
def auto_contrast_np(img, clip_hist_percent=1):
    # 将图像转换为 8 位范围
    ...
    min_gray = 0
    while min_gray < hist_size and accumulator[min_gray] < clip_hist_percent:
        min_gray += 1

    max_gray = hist_size - 1
    while max_gray >= 0 and accumulator[max_gray] >= (maximum - clip_hist_percent):
        max_gray -= 1
    ...
    auto_contrasted = cv2.convertScaleAbs(img, alpha=alpha, beta=beta)
    
    return auto_contrasted
...
class WeakStrongAugment(object):
    ...
    def __call__(self, sample):
        ...
        # image_strong = auto_contrast_np(image)
        image_strong, label_strong = cutout_gray(image,label,p=0.5)
        # image_strong, label_strong = image,label
